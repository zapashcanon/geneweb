...
let unpack ~force_unpack ~verbose =
  if force_unpack || not (Sys.file_exists root) then begin
    Array.iter
      begin
        fun dir -> mkdir_p ~verbose (path dir)
      end
      Data.directories;
...
(** For script execution, run: cat <script.ml> | [ GWREPL_PPF=/dev/null ]
    [ GWREPL_VERBOSE=1 ] [ GWREPL_FORCE_UNPACK=1 ] [ GWREPL_NOPROMPT=1 ]
    gwrepl.exe [scrip_arg1] ... For interactive toplevel, run: gwrepl.exe *)
let () =
  let ppf =
