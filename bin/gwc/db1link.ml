...
let unique_string gen x =
  ...
  with Not_found ->
    (* string not found *)
    if gen.g_scnt = Array.length gen.g_base.c_strings then begin
...
let insert_undefined gen key =
  ...
  let x, ip =
    ...
    with Not_found ->
      ...
      if key.pk_first_name <> "?" && key.pk_surname <> "?" then begin
...
let insert_person gen so =
  ...
  let x, ip =
    ...
    with Not_found ->
      ...
      gen.g_pcnt <- gen.g_pcnt + 1;
      (* strore names locally *)
      if so.first_name <> "?" && so.surname <> "?" then begin
  ...
  if gen.g_def.(ip) then begin
    ...
    then
      Printf.printf "as name: \"%s%s %s\"\n"
        ...
        (p_surname gen.g_base x);
    flush stdout;
    check_error gen (* else set it as defined *)
  end
  ...
  if not gen.g_errored then begin
    let empty_string = unique_string gen "" in
  ...
  let rec loop fevents fam =
    ...
    | evt :: l -> (
      ...
      | Efam_Marriage ->
        ...
        let fam =
          { fam with
            relation = (if nsck_std_fields then NoSexesCheckMarried else Married)
          ; marriage = evt.efam_date
          ; marriage_place = evt.efam_place
          ; marriage_note = evt.efam_note
          ; marriage_src = evt.efam_src
      ...
      | Efam_Annulation | Efam_PACS ->
        if !found_marriage then loop l fam
          ...
          let fam =
            { fam with
              relation =
                (if nsck_std_fields then NoSexesCheckNotMarried else NoMention)
            ; marriage = evt.efam_date
            ; marriage_place = evt.efam_place
            ; marriage_note = evt.efam_note
            ; marriage_src = evt.efam_src
      ...
      | Efam_NoMarriage ->
        if !found_marriage then loop l fam
          ...
          let fam =
            { fam with
              relation =
                (if nsck_std_fields then NoSexesCheckNotMarried else NotMarried)
            ; marriage = evt.efam_date
            ; marriage_place = evt.efam_place
            ; marriage_note = evt.efam_note
            ; marriage_src = evt.efam_src
...
let insert_family gen co fath_sex moth_sex witl fevtl fo deo =
  ...
  let fsources =
    unique_string gen (if fo.fsources = "" then !default_source else fo.fsources)
  in
  (* extend arrays if needed *)
  new_ifam gen;
...
let insert_notes fname gen key str =
  ...
  | Some ip ->
    let p = poi gen.g_base ip in
    if sou gen.g_base p.m_notes <> "" then begin
      Printf.printf "\nFile \"%s\"\n" fname;
