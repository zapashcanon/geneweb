...
let copy_decode s i1 i2 =
  ...
  let rec loop_copy t i j =
    ...
    else if i = i2 - 1 && s.[i] <> '_' then begin
      Bytes.set t j s.[i];
  ...
  let dmy2 year2 i =
    ...
    | Some i ->
      ...
      begin
        ...
        | None ->
          if month2 < 1 || month2 > 13 then error 4 else ((0, month2, year2), i)
      end
    | None -> ((0, 0, year2), i)
  in
  let date =
  ...
  | x :: l' -> begin
    ...
    | '?' | ' ' ->
      ...
      let x, occ =
        ...
        | Some i -> begin
          try (String.sub x 0 i, make_int x (succ i)) with Not_found -> (x, 0)
        end
        | None -> (x, 0)
      in
      (x, occ, l')
...
let scan_title t =
  ...
  if i <> String.length t then failwith t
    ...
    ; t_date_end = Adef.cdate_of_od date_end
    ; t_nth = nth
    }
...
let get_mar_date str = function
  | x :: l ->
    let mar, l =
      match x.[0] with
      | '+' ->
        ( ( if String.length x > 1 then Adef.cdate_of_od (date_of_string x 1)
    ...
    let relation, l =
      ...
      | "#noment" :: c :: l when String.length c = 2 -> decode_sex NoMention c l
...
let set_infos fn sn occ sex comm_psources comm_birth_place str u l =
  ...
  let mort =
    ...
    | None, _ | _, Some _ | Some None, _ -> begin
  ...
  in
  let baptism =
    match baptism with None -> Adef.cdate_None | Some x -> Adef.cdate_of_od x
  in
