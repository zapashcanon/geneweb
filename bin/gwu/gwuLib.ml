  ...
  if !old_gw then
    let rec loop pevents =
      ...
      | evt :: events -> (
        ...
        | Epers_Cremation ->
          if sou base evt.epers_note <> "" || evt.epers_witnesses <> [||] then
            true
          else loop events
        | _ -> true )
    in
...
let print_title opts base t =
  ...
  if t.t_nth <> 0 then Printf.ksprintf (oc opts) ":"
    ...
    match t_date_end with Some _ -> Printf.ksprintf (oc opts) ":" | _ -> ()
  end;
...
let print_pevents_for_person opts base gen p =
  ...
  if pevents <> [] && surn <> "?" && fnam <> "?" then begin
    Printf.ksprintf (oc opts) "\n";
...
let print_family opts base gen m =
  ...
  if (not !old_gw) && get_fevents fam <> [] then begin
...
let print_notes_for_person opts base gen p =
  ...
  then begin
    ...
    if put_events_in_notes base p then begin
      let rec loop pevents =
        match pevents with
        | [] -> ()
        | evt :: events -> (
    ...
    if (not !old_gw) && opts.source = None then
      List.fold_left
        begin
          fun acc e ->
            let acc =
              if opts.no_notes <> `nnn then sou base e.epers_note :: acc
              else acc
            in
            let acc =
  ...
  | _ ->
    if err_same_sex then
      ...
      begin
        ...
        | Some fath, Some moth ->
          if get_sex fath = Male && get_sex moth = Female then begin
...
let print_relations opts base gen ml =
  ...
  let rec loop = function
    ...
    | (p, if_def) :: pl ->
      let def_p = ref [] in
      if get_rparents p <> [] && gen.per_sel (get_iper p) then begin
...
let rec find_ancestors base surn p list =
  ...
  | Some ifam ->
    ...
    then p :: list
      ...
      in
      if eq_istr (get_surname moth) surn then find_ancestors base surn moth list
      else list
  | None -> p :: list
...
let mark_one_connex_component base mark ifam =
  ...
  let set_mark x =
    let test_action loop () ifam =
      if Gwdb.Marker.get mark ifam = BeingScanned then begin
          ...
          let ml =
            List.fold_right
              (fun ifam ml ->
                ...
                in
                if empty_family base m then begin
                  Gwdb.Marker.set gen.fam_done m.m_ifam true;
                  ml
                end
                else m :: ml )
