...
let print_and_cut_if_too_big oc str =
  let rec loop i =
    if i < String.length str then begin
      ...
      let i =
        if i > 700 && String.length str - i > 750 then begin
          Printf.fprintf oc " ... ";
                ...
                begin
                  fun fname ->
                    Mutil.input_lexicon lang ht
                      begin
                        fun () -> Secure.open_in (Util.search_in_assets fname)
                      end
                end
                !lexicon_list;
              ht
  ...
  try
    ...
    let rec loop changed r list =
      ...
      | exception End_of_file ->
        close_in ic;
        let list =
          List.sort (fun (_, (t1, _, _)) (_, (t2, _, _)) -> compare t2 t1) list
        in
        (list, r, changed)
    in
    loop false ATnormal []
    ...
    if (not wizard) && not friend then NoAuth
      ...
      let u, p =
        ...
        | Some i ->
          ...
          let p = String.sub passwd1 (i + 1) (String.length passwd1 - i - 1) in
          (u, p)
        | None -> ("", passwd)
      in
...
let make_conf from_addr request script_name env =
  ...
  let conf =
    ...
    ; border =
        begin
          match Util.p_getint env "border" with Some i -> i | None -> 0
        end
    ; n_connect = None
    ; today =
        { day = tm.Unix.tm_mday
    ...
    ; image_prefix =
        ...
        else if !Wserver.cgi then begin
    ...
    if auth <> "" then
      ...
      | Some ic ->
        ...
        begin
          try
            let rec loop () =
              if auth = input_line ic then begin
                close_in ic;
                let s =
                  try
...
let geneweb_server () =
  ...
  Wserver.f GwdLog.syslog !selected_addr !selected_port !conn_timeout
    (if Sys.unix then !max_clients else None)
    connection
...
let manage_cgi_timeout tmout =
  if tmout > 0 then
    let _ =
      Sys.signal Sys.sigalrm (Sys.Signal_handle (cgi_timeout printer_conf tmout))
    in
            ...
            List.map
              begin
                fun pname ->
                  ...
                  if (not unsafe) && not (GwdPluginMD5.allowed dir) then
                    failwith s;
                ...
                begin
                  fun pname ->
                    try
                      ...
                      if unsafe then unsafe_plugins := !unsafe_plugins @ [ s ];
                      if force then
                        forced_plugins := !forced_plugins @ [ pname ];
                      plugins := !plugins @ [ s ]
                    with Not_found ->
                      raise
                        (Register_plugin_failure
                           (pname, `string "Missing plugin") )
                end
                deps
          end
      end
...
let main () =
  ...
  let speclist =
    ...
    ; ( "-log_level"
      , Arg.Set_int GwdLog.verbosity
      , {|<N> Send messages with severity <= <N> to syslog (default: |}
        ^ string_of_int !GwdLog.verbosity
        ^ {|).|} )
    ; ( "-robot_xcl"
