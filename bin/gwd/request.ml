        ...
        let add_tl t =
          tl :=
            let rec add_rec = function
              | t1 :: tl1 ->
                if eq_istr t1.t_ident t.t_ident && eq_istr t1.t_place t.t_place
                then t1 :: tl1
                else t1 :: add_rec tl1
              | [] -> [ t ]
            in
      ...
      fun (p, tl) ->
        ...
        begin
          ...
          | t :: _ ->
            ...
            List.iter
              (fun t -> Output.print_string conf (one_title_text base t))
              tl
        end;
...
let w_lock ~onerror fn conf (base_name : string option) =
  ...
  Lock.control (Mutil.lock_file bfile) true
    ...
    let process () =
      ...
      then begin
        ...
        if not @@ try_plugin plugins conf bfile m then
          begin
            if List.assoc_opt "counter" conf.base_env <> Some "no" then begin
              match
            ...
            | "A" -> Perso.print_ascend |> w_person |> w_base
            ...
            | "CAL" -> fun conf _ -> Hutil.print_calendar conf
            ...
            | "MRG_IND" -> w_wizard @@ w_lock @@ w_base @@ MergeIndDisplay.print
            | "MRG_IND_OK" ->
                     ...
                     | Some "input" | None ->
                       ...
                       begin
                         ...
                         | None -> (
                           ...
                           | Some fn, None -> Some.first_name_print conf base fn
                           | None, Some sn ->
