...
and compare_month date1 date2 =
  ...
  | Tint 0, Tint _ ->
    if field date1 "prec" = Tsafe "after" then Tint 1 else cmp_prec date1 date2
  | Tint _, Tint 0 ->
...
and module_DATE conf =
  ...
  let sub =
    func_arg2_no_kw
      begin
        fun d1 d2 -> mk_dmy @@ Date.time_elapsed (to_dmy d2) (to_dmy d1)
      end
  in
  let calendar =
    func_arg2_no_kw
...
let module_OPT =
  let map =
    func_arg2_no_kw
      begin
        fun fn -> function Tnull -> Tnull | x -> (unbox_fun fn) x
      end
  in
  Tpat
    begin
      function "map" -> map | _ -> raise Not_found
    end
