        ...
        then
          ...
          | Some ifam ->
            ...
            begin
              ...
              | Left zil ->
                Left
                  ((get_father cpl, z) :: (get_mother cpl, Sosa.inc z 1) :: zil)
              | Right z -> Right z
            end
          | None -> gene_find zil
      end
    ...
    if i > nb_persons then ()
      ...
      if Sosa.eq sosa_num Sosa.zero then ()
        ...
        | Some ifam ->
          ...
          let len =
            if not @@ Gwdb.Marker.get mark (get_father cpl) then begin
          ...
          in
          let len =
            if not @@ Gwdb.Marker.get mark (get_mother cpl) then begin
