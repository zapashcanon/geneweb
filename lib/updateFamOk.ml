  ...
  let children, ext =
    let rec loop i ext =
      match
        try
          Some
            (reconstitute_parent_or_child conf
               ("ch" ^ string_of_int i)
               surname )
        with Failure _ -> None
      with
      ...
      | _ ->
        let rec loop i =
          if i = Array.length ndes.children then Gwdb.insert_string base ""
            ...
            else if sou base (get_origin_file (foi base cifams.(0))) <> "" then
        ...
        let rec loop i =
          if i = -1 then print_add o_conf base
            ...
            if Array.exists (( = ) ifam) mfam then (
              ...
              let sdes =
                { children =
                    Array.append (gen_descend_of_family f).children [| child |]
                }
              in
