      ...
      if c > 0 then
        ...
        ^ ( if cw > 0 then
            Printf.sprintf ", %s %s"
              ...
              ( if conf.wizard then
                Printf.sprintf "<a href=\"%sm=CONN_WIZ\">%d</a>" (commd conf) cw
      ...
      | None -> (
        ...
        | Some s3 ->
          let s21, s22 =
            ...
            (String.sub s2 0 i, String.sub s2 (i + 1) (String.length s2 - i - 1))
          in
 ...
 fun conf ifun env ->
  ...
  and eval_apply env ep loc f vl =
    ...
    | None -> (
      ...
      | "red_of_hsv", [ VVstring h; VVstring s; VVstring v ] -> begin
        try
          let r, _, _ = rgb_of_str_hsv h s v in
          string_of_int r
        with Failure _ -> "red_of_hsv bad params"
      end
      | "green_of_hsv", [ VVstring h; VVstring s; VVstring v ] -> begin
        try
          let _, g, _ = rgb_of_str_hsv h s v in
          string_of_int g
        with Failure _ -> "green_of_hsv bad params"
      end
      | "blue_of_hsv", [ VVstring h; VVstring s; VVstring v ] -> begin
        try
    ...
    with Not_found -> (
      ...
      | [ "include"; templ ] -> begin
        ...
        | Some (_, fname) -> begin
          ...
          | Some astl ->
            let () =
              Templ_parser.(included_files := (templ, astl) :: !included_files)
            in
            Util.include_begin conf (Adef.safe fname);
            print_ast_list env ep astl;
            Util.include_end conf (Adef.safe fname)
...
let copy_from_templ : config -> Adef.encoded_string env -> in_channel -> unit =
 fun conf env ic ->
  ...
  Templ_parser.wrap ""
    begin
      fun () -> interp_ast conf ifun env () astl
    end
