...
let relationship_and_links base ri b ip1 ip2 =
  ...
  if i1 = i2 then (1.0, [])
    ...
    let insert u =
      ...
      reset u;
      if v >= Array.length ri.queue then begin
    ...
    let treat_parent ip_from u y =
      ...
      if u.anc_stat1 = IsAnc && ty.anc_stat1 <> IsAnc then begin
        ty.anc_stat1 <- IsAnc;
        incr nb_anc1
      end;
      if u.anc_stat2 = IsAnc && ty.anc_stat2 <> IsAnc then begin
        ty.anc_stat2 <- IsAnc;
        incr nb_anc2
      end;
      ...
      if b && not ty.elim_ancestors then begin
    ...
    let treat_ancestor u =
      ...
      if b && contribution <> 0.0 && not tu.elim_ancestors then begin
        tops := u :: !tops;
