...
(** [control ~onerror lname wait f] opens file [lname], puts a write lock on it
    ...
    it will be blocked until these lock is removed. Otherwise it will fail, and
    function [onerror] will be called. If flag [no_lock_flag] is set then
    returns [f ()] immediatly. *)
val control : onerror:(unit -> 'a) -> string -> bool -> (unit -> 'a) -> 'a

(** Tries to call [control] without blocking. If it fail (lock is put) then call
    again [control] and waits untill lock is removed. If it fails with another
    reason calls [onerror]. *)
