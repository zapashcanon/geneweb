...
let lock_file bname =
  let bname =
    if Filename.check_suffix bname ".gwb" then Filename.chop_suffix bname ".gwb"
    else bname
  in
  bname ^ ".lck"
...
let string_of_int_sep sep x =
  ...
  let _ =
    List.fold_left
      (fun (i, j) c ->
        Bytes.set s j c;
        if i < len - 1 && (len - 1 - i) mod 3 = 0 then begin
...
let read_or_create_channel_windows ?magic ?(wait = false) fname read write =
  let _ = wait in
...
let read_or_create_channel_unix ?magic ?(wait = false) fname read write =
  assert (Secure.check fname);
  let fd = Unix.openfile fname [ Unix.O_RDWR; Unix.O_CREAT ] 0o666 in
  begin
...
let read_or_create_channel ?magic ?(wait = false) fname read write =
  if Sys.os_type = "Unix" then
    read_or_create_channel_unix ~magic ~wait fname read write
  else read_or_create_channel_windows ~magic ~wait fname read write
