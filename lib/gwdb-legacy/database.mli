...
(** [make bname particles ((persons, ascendants, unions) (families, couples,
    descendants) strings base_notes)]
    returns initialised with giving data [dsk_base]. This function is called
    exclusively for database creating purpose. It means that, it contains only
    data without functionalities. Either call [opendb] on existing database or
    call [Gwdb.make], if you want to make requests. *)
val make :
     string
  -> string list
