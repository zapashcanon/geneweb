...
let old_persons_of_first_name_or_surname base_data params =
    ...
    fun () ->
      ...
      | None ->
        ...
        Hashtbl.iter
          (fun _i p ->
            let istr1 = proj p in
            if not @@ IstrTree.mem istr1 !bt then bt := IstrTree.add istr1 0 !bt
            )
          person_patches;
        btr := Some !bt;
        !bt
  ...
  let cursor str =
    IstrTree.key_after
      (fun key -> Dutil.compare_snames base_data str (base_data.strings.get key))
      (bt_patched ())
  in
...
let opendb bname =
  ...
  in
  let nbp_read () =
    if Sys.file_exists nbp_fname then begin
