    ...
    let rec loop = function
      ...
      | (nl, cs) :: rest ->
        let rec loop1 beg = function
          | (nl1, cs1) :: rest1 ->
            if S.is_empty (S.inter cs cs1) then loop1 ((nl1, cs1) :: beg) rest1
            else loop ((nl @ nl1, S.union cs cs1) :: List.rev_append beg rest1)
          | [] -> (nl, cs) :: loop rest
        in
        loop1 [] rest
    in
    ...
    | Some (parents, max_parent_colspan) ->
      ...
      let children =
        if children = [] then [ { elem = Nothing; span = new_span_id () } ]
          ...
          List.map (fun n -> { elem = Elem n; span = new_span_id () }) children
      in
...
let group_ghost t =
  for i = 0 to Array.length t.table - 2 do
    for j = 1 to Array.length t.table.(0) - 1 do
      ...
      | Ghost x, Ghost _ ->
        if t.table.(i + 1).(j - 1).elem = t.table.(i + 1).(j).elem then begin
          t.table.(i).(j) <- { elem = Ghost x; span = t.table.(i).(j - 1).span };
          if i > 0 then t.table.(i - 1).(j).span <- t.table.(i - 1).(j - 1).span
        end
      | _ -> ()
    done
  done
...
let push_to_right t i j1 j2 =
  ...
  let rec loop j =
    if j = j2 then j - 1
      ...
      in
      if jj4 < j2 && jj2 < jj3 then begin
        exch_blocks t ii i jj1 jj2 jj3 jj4;
        loop (jj4 + 1)
      end
      else if jj4 < j2 && jj1 = ini_jj1 && jj2 <= jj4 then begin
  ...
  let rec loop j =
    if j = j1 then j + 1
      ...
      let ii, jj1, jj2, jj3, jj4 =
        find_block_with_parents t i jj1 jj2 jj3 jj4
      in
      if jj1 > j1 && jj2 < jj3 then begin
        exch_blocks t ii i jj1 jj2 jj3 jj4;
        loop (jj1 - 1)
      end
      else if jj1 > j1 && jj4 = ini_jj4 && jj3 >= jj1 then begin
        mirror_block t ii i jj1 jj4;
        loop (jj1 - 1)
      end
      else j + 1
                ...
                t.table.(i1).(l) <-
                  ( if
                    ...
                    || t.table.(i1 - 1).(l - 1).span
                       <> t.table.(i1 - 1).(l).span
                  then { elem = Ghost (new_ghost_id ()); span = new_span_id () }
...
let fall2_cool_right t i1 i2 j1 j2 =
  ...
  let rec loop j =
    ...
    else if t.table.(i2 - 1).(j).span = old_span then begin
    ...
    let i1 =
      let rec loop i =
        if i < 0 then 0
          ...
          match t.table.(i).(j).elem with Ghost _ -> loop (i - 1) | _ -> i + 1
      in
      loop (i - 1)
    in
    let separated1 =
...
let try_shorten_too_long t i j =
  ...
  | Ghost _ ->
    ...
    let i2 =
      let rec loop i =
        if i = Array.length t.table then i
        else match t.table.(i).(j).elem with Nothing -> loop (i + 1) | _ -> i
      in
      loop i1
    in
    let separated_left =
