      ...
      fun rp accu ->
        ...
        | Some ip1, Some ip2 ->
          ...
          begin
            ...
            | _, _ ->
              (rel ^<^ transl conf ":" ^<^ fath ^^^ ", " ^<^ moth) :: accu
          end
        | Some ip, None ->
          let p = person_of_iper conf base ip in
  ...
  Array.fold_right
    begin
      fun (ip, wk) accu ->
        ...
        let kind =
          Util.string_of_witness_kind conf (get_sex @@ poi base ip) wk
        in
        if (witn :> string) = "" then
          (kind ^^^ transl conf ":" ^<^ witn) :: accu
        else accu
    end
    witnesses []
  |> fun s ->
...
let string_of_epers_name conf epers_name =
  ...
  | Epers_Name n ->
    Adef.safe @@ Utf8.capitalize_fst (escape_html n : Adef.safe_string :> string)
...
and eval_str_gen_record conf base env (bef, aft, p_auth) :
  ...
  | "pevent_name" -> begin
    ...
    | Vpevent (bef, aft) ->
      aux' true bef aft (fun conf x -> string_of_epers_name conf x.epers_name)
    | _ -> raise Not_found
  end
  | "pevent_date" -> begin
    match get_env "pevent" env with
  ...
  | "fevent_name" -> begin
    ...
    | Vfevent (bef, aft, m_auth) ->
      aux' m_auth bef aft (fun conf x -> string_of_efam_name conf x.efam_name)
    | _ -> raise Not_found
  end
  | "fevent_date" -> begin
    match get_env "fevent" env with
      ...
      let rec loop i list =
        ...
        | gr :: l ->
          let env =
            ...
            :: ( "wizard"
               , Vstring (gr.HistoryDiff.wizard : Adef.safe_string :> string) )
            :: env
          in
          List.iter (print_ast env xx) al;
          loop (i + 1) l
  ...
  | Some ("SUM" | "DIFF") -> begin
    ...
    | Some file when file <> "" ->
      ...
      let before, after =
        ...
        | Some o, Some n ->
          let o = if o < 0 then 0 else if o > len - 1 then len - 1 else o in
          let n = if n < 0 then 0 else if n > len - 1 then len - 1 else n in
          (o, n)
        | _ -> (0, 0)
      in
