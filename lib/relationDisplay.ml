  ...
  | 2 ->
    ...
    let is =
      if nb_fields txt = 6 then
        match get_piece_of_branch conf base info (1, 1) with
        | [ ip1 ] -> if get_sex (pget conf base ip1) = Male then is else is + 3
        | _ -> (* must be a bug *) is
      else is
    in
    nth_field txt is |> Adef.safe
  ...
  | 3 ->
    ...
    let is =
      if nb_fields txt = 12 then
        ...
        | [ ip1; ip2 ] ->
          let is = if get_sex (pget conf base ip1) = Male then is else is + 6 in
  ...
  | 1 ->
    ...
    let is =
      if nb_fields txt = 4 then
        ...
        match get_piece_of_branch conf base info (1, 1) with
        | [ ip1 ] -> if get_sex (pget conf base ip1) = Male then is else is + 2
        | _ -> (* must be a bug *) is
      else is
    in
    nth_field txt is |> Adef.safe
