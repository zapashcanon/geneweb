...
let get_burial_text conf p p_auth =
  ...
  let on_burial_date =
    ...
    | Buried cod -> begin
      match (p_auth, Adef.od_of_cdate cod) with
      | true, Some d -> begin
        match List.assoc_opt "long_date" conf.base_env with
        | Some "yes" ->
          DateDisplay.string_of_ondate ~link:false conf d
      ...
      let new_ipl =
        List.fold_left
          (fun ipl ip ->
            ...
            else if lev <= max_level then begin
    ...
    List.fold_left
      (fun gpl gp ->
        ...
        | GP_person (n, ip, _) ->
          ...
          if Sosa.eq m Sosa.zero then begin
          ...
          let rec loop' j =
            if j = len then loop (i + 1)
              ...
              if get_iper sp1 = get_iper sp2 then
                ...
                if not (List.mem (ifam2, ifam2) fexcl) then DupFam (ifam1, ifam2)
                else loop' (j + 1)
              else
  ...
  let rec loop lev sosa p surn dp =
    if Gwdb.Marker.get mark (get_iper p) = 0 then ()
      ...
      | Some ifam ->
        ...
        if not (is_hidden fath) then begin
...
let build_list_eclair conf base v p =
  ...
  let add_person p surn =
    if Gwdb.Marker.get mark (get_iper p) then ()
      ...
      add_surname p surn (get_birth_place p) (Adef.od_of_cdate (get_birth p));
...
and eval_simple_bool_var conf base env =
  ...
  | "browsing_with_sosa_ref" -> begin
    match get_env "sosa_ref" env with
    | Vsosa_ref v -> v <> None
    | _ -> raise Not_found
  end
  | "has_comment" | "has_fnotes" -> begin
    match get_env "fam" env with
...
and eval_simple_str_var conf base env (_, p_auth) = function
  ...
  | "divorce_date" -> begin
    match get_env "fam" env with
    | Vfam (_, fam, _, m_auth) when mode_local env -> begin
      match get_divorce fam with
      | Divorced d -> begin
  ...
  | "slash_marriage_date" -> begin
    match get_env "fam" env with
    | Vfam (_, fam, _, m_auth) -> begin
      match (m_auth, Adef.od_of_cdate (get_marriage fam)) with
...
and eval_compound_var conf base env ((a, _) as ep) loc = function
  ...
  | "number_of_descendants" :: sl -> begin
    ...
    | Vint i -> begin
      ...
      | Vdesclevtab t ->
        ...
        let cnt =
          Gwdb.Collection.fold
            (fun cnt ip -> if Gwdb.Marker.get m ip <= i then cnt + 1 else cnt)
            0 (Gwdb.ipers base)
        in
  ...
  | "number_of_descendants_at_level" :: sl -> begin
    match get_env "level" env with
    | Vint i -> begin
      match get_env "desc_level_table" env with
      | Vdesclevtab t ->
...
and eval_anc_by_surnl_field_var conf base env ep info =
  ...
  | Branch (_, db, de, place, p, sosa_list, loc) -> (
    function
    | "date_begin" :: sl -> begin
      match db with Some d -> eval_date_field_var conf d sl | None -> null_val
    end
    | "date_end" :: sl -> begin
      match de with Some d -> eval_date_field_var conf d sl | None -> null_val
    end
...
and eval_person_field_var conf base env ((p, p_auth) as ep) loc = function
  ...
  | "burial_date" :: sl -> begin
    match get_burial p with
    | Buried cod when p_auth -> begin
  ...
  | "cremated_date" :: sl -> begin
    match get_burial p with
    | Cremated cod when p_auth -> begin
      match Adef.od_of_cdate cod with
      | Some d -> eval_date_field_var conf d sl
      | None -> null_val
    end
  ...
  | [ "init_cache"; nb_asc; from_gen_desc; nb_desc ] -> begin
    try
  ...
  | "marriage_date" :: sl -> begin
    match get_env "fam" env with
    | Vfam (_, fam, _, true) -> begin
...
and eval_date_field_var conf d = function
  ...
  | [ "day2" ] -> begin
    ...
    | Dgreg (dmy, _) -> begin
      ...
      | OrYear dmy2 | YearInt dmy2 ->
        if dmy2.day2 = 0 then null_val else VVstring (string_of_int dmy2.day2)
      | _ -> null_val
    end
    | _ -> null_val
  end
  ...
  | [ "gregorian_day" ] -> begin
    match d with
    | Dgreg (dmy, _) -> VVstring (string_of_int (Calendar.sdn_of_gregorian dmy))
    | _ -> VVstring ""
  end
  | [ "month" ] -> begin
    match d with
...
and eval_bool_event_field base (p, p_auth) (_, date, place, note, src, w, isp) =
  ...
  | "computable_age" ->
    if p_auth then
      match Adef.od_of_cdate (get_birth p) with
      | Some (Dgreg (d, _)) -> not (d.day = 0 && d.month = 0 && d.prec <> Sure)
      | Some _ | None -> (
        match Adef.od_of_cdate (get_baptism p) with
        | Some (Dgreg (d, _)) -> not (d.day = 0 && d.month = 0 && d.prec <> Sure)
        | Some _ | None -> false )
...
and eval_bool_person_field conf base env (p, p_auth) = function
  ...
  | "wedding_birthday" -> begin
    match get_env "fam" env with
    | Vfam (_, fam, _, m_auth) -> begin
      match (get_relation fam, get_divorce fam) with
      | (Married | NoSexesCheckMarried), NotDivorced -> begin
        match (m_auth, Adef.od_of_cdate (get_marriage fam)) with
        | true, Some (Dgreg (d, _)) ->
  ...
  | "computable_death_age" ->
    if p_auth then
      ...
      | ( Some (Dgreg (({ prec = Sure | About | Maybe } as d1), _))
        ...
        when d1 <> d2 ->
        let a = Date.time_elapsed d1 d2 in
        a.year > 0 || (a.year = 0 && (a.month > 0 || (a.month = 0 && a.day > 0)))
      | _ -> false
    else false
  | "computable_marriage_age" -> begin
    match get_env "fam" env with
    | Vfam (_, fam, _, m_auth) ->
      if m_auth then
        match
          (Adef.od_of_cdate (get_birth p), Adef.od_of_cdate (get_marriage fam))
        with
        | ( Some (Dgreg (({ prec = Sure | About | Maybe } as d1), _))
          , Some (Dgreg (({ prec = Sure | About | Maybe } as d2), _)) ) ->
          let a = Date.time_elapsed d1 d2 in
  ...
  | "has_aliases" ->
    if (not p_auth) && is_hide_names conf p then false else get_aliases p <> []
            ...
            | Event.Pevent pname -> begin
              ...
              | Epers_Cremation ->
                if Array.length wl > 0 then true
                  ...
                  then true
                  else loop events nb_birth nb_bapt nb_deat nb_buri nb_marr
              | _ -> true
            end
            | Fevent fname -> (
              match fname with
              | Efam_Engage | Efam_Marriage | Efam_NoMention | Efam_NoMarriage
                ->
                let nb_marr = succ nb_marr in
                if nb_marr > nb_fam then true
                else loop events nb_birth nb_bapt nb_deat nb_buri nb_marr
              | Efam_Divorce | Efam_Separated ->
                if p <> "" || n <> "" || s <> "" || Array.length wl > 0 then
                  true
                else loop events nb_birth nb_bapt nb_deat nb_buri nb_marr
              | _ -> true ) )
        in
...
and eval_str_person_field conf base env ((p, p_auth) as ep) = function
  ...
  | "age" -> begin
    match (p_auth, Adef.od_of_cdate (get_birth p), get_death p) with
        ...
        if Gwdb.Marker.get tab i then ()
          ...
          for i = 0 to Array.length (get_family u) - 1 do
            ...
            for i = 0 to Array.length (get_children des) - 1 do
              mark_descendants (len + 1) (pget conf base (get_children des).(i))
            done
          done
        end
      in
  ...
  | "marriage_age" -> begin
    ...
    | Vfam (_, fam, _, m_auth) ->
      if m_auth then
        ...
        | ( Some (Dgreg (({ prec = Sure | About | Maybe } as d1), _))
          , Some (Dgreg (({ prec = Sure | About | Maybe } as d2), _)) ) ->
          Date.time_elapsed d1 d2 |> DateDisplay.string_of_age conf |> safe_val
        | _ -> null_val
      else null_val
    | _ -> raise Not_found
  end
    ...
    if p_auth then
      ...
      let list =
        ...
        if first_name <> "?" && surname <> "?" then
          (first_name ^ " " ^ surname |> Name.lower |> Util.escape_html) :: list
        else list
      in
      if list <> [] then
        "<ul>"
    ...
    | _ -> (
      ...
      | Vfam (ifam, _, _, _) ->
        let baseprefix =
          ...
          | _ -> conf.command
        in
        string_of_int (!GWPARAM_ITL.nb_children baseprefix ifam) |> str_val
      | _ ->
        Array.fold_left
          (fun n ifam -> n + Array.length (get_children (foi base ifam)))
          0 (get_family p)
  ...
  | "surname_begin" ->
    if (not p_auth) && is_hide_names conf p then null_val
      ...
      p_surname base p |> surname_particle base |> Util.escape_html |> safe_val
  | "surname_end" ->
    if (not p_auth) && is_hide_names conf p then str_val "x"
    else
      p_surname base p
...
and eval_family_marriage_field_var fam = function
  ...
  | [ "nb_witnesses_other" ] ->
    VVstring
      (get_nb_marriage_witnesses_of_kind fam Def.Witness_Other |> string_of_int)
  | _ -> raise Not_found
...
let print_foreach conf base print_ast eval_expr =
  ...
  let print_foreach_child env al ep = function
    | Vfam (ifam, fam, (ifath, imoth, isp), _) -> begin
      match get_env "f_link" env with
      | Vbool _ ->
        let baseprefix =
          match get_env "baseprefix" env with
  ...
  let print_foreach_family env al ini_ep (p, _) =
    ...
    | Vbool _ ->
      ...
      |> ignore
    | _ ->
      if Array.length (get_family p) > 0 then begin
        let rec loop prev i =
...
let limit_by_tree conf =
  match
    Option.map int_of_string (List.assoc_opt "max_anc_tree" conf.base_env)
  with
  | Some x -> max 1 x
  | None -> 7
