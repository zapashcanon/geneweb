...
and eval_event_str conf base env fam =
  ...
  | Vint i -> begin
    try
      ...
      let s =
        String.concat ", "
          ([ name; date; (place :> Adef.safe_string); note; src ] :> string list)
      in
...
and eval_fwitness_kind env fam =
  match get_env "cnt" env with
  | Vint i ->
    let e = try Some (List.nth fam.fevents (i - 1)) with Failure _ -> None in
    begin
      match e with
      | Some e -> begin
        match get_env "wcnt" env with
...
and eval_event_date env fam s =
  let od =
    family_events_opt env fam >>= fun e -> Adef.od_of_cdate e.efam_date
  in
  eval_date_var od s
...
and eval_create c = function
  | "birth_day" ->
    ...
    @@ begin
         ...
         | Update.Create
             (_, Some { ci_birth_date = Some (Dgreg ({ day = d }, _)) })
           when d <> 0 ->
           string_of_int d
         | _ -> ""
       end
  ...
  | "death_day" ->
    ...
    @@ begin
         ...
         | Update.Create
           ...
           if dmy.day <> 0 then string_of_int dmy.day else ""
         | Update.Create
             (_, Some { ci_death_date = Some (Dgreg ({ day = d }, _)) })
           when d <> 0 ->
           string_of_int d
         | _ -> ""
       end
