...
let compare_title_dates conf base (x1, t1) (x2, t2) =
  match
    ...
    , ( get_birth x2
      ...
      , get_death x2 ) )
  with
  | (_, Some (Dgreg (d1, _)), _, _), (_, Some (Dgreg (d2, _)), _, _) -> begin
  ...
  | _ -> (
    match
      (date_interval conf base JustSelf x1, date_interval conf base JustSelf x2)
    with
...
let select_all_with_place conf base place =
  ...
  let select x t =
    if Name.lower (sou base t.t_place) = place then begin
      clean_place := sou base t.t_place;
      list := (x, t) :: !list
    end
  in
...
let select_title conf base ~absolute title =
  ...
  let add_place t =
    ...
    then (
      ...
      if not (StrSet.mem pn !set) then begin
        clean_name := tn;
...
let select_place conf base place =
  ...
  let add_title t =
    ...
    if Name.lower pn = place2 then
      let tn = sou base t.t_ident in
      if not (List.mem tn !list) then begin
        clean_name := pn;
        list := tn :: !list
      end
  in
