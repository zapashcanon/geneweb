...
let infer_death_from_parents conf base fam =
  ...
  let aux = function
    ...
    | _ -> (
      match
        (Adef.od_of_cdate @@ get_birth moth, Adef.od_of_cdate @@ get_birth fath)
      with
  ...
  | (Death (_, d1) as a), (Death (_, d2) as b) -> begin
        ...
        if i = len then DontKnowIfDead
          ...
          | _ ->
            let death =
              ...
              loop_children 0
            in
            if death = OfCourseDead then OfCourseDead else loop_families (i + 1)
      in
      loop_families 0
    in
    if death <> DontKnowIfDead then death
...
let print_aux conf param (value : Adef.encoded_string)
  ...
  List.iter
    begin
      fun (x, v) -> Util.hidden_textarea conf x v
    end
    conf.henv;
  List.iter
    begin
      ...
      begin
        ...
        | Some m -> begin
          ...
          | Some d ->
            ...
            if d.day >= 1 && d.day <= 31 && d.month >= 1 && d.month <= 13 then
              Some d
            else bad_date conf d
          | None ->
            let d = { day = 0; month = m; year = y; prec; delta = 0 } in
            if d.month >= 1 && d.month <= 13 then Some d else bad_date conf d
        end
