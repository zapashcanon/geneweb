...
let add_common_parent base ip1 ip2 set =
  ...
  | Some ifam1, Some ifam2 ->
    ...
    if get_father cpl1 = get_father cpl2 then Dag.Pset.add (get_father cpl1) set
...
let excl_faml conf base =
  let rec loop list i =
    ...
    | None -> (
      ...
      | Some p ->
        let n =
          p_getint conf.env ("fef" ^ string_of_int i) |> Option.value ~default:0
        in
        let list =
    ...
    let rec loop1 newvertexlist = function
      | vertex :: vertexlist ->
        let rec loop2 result = function
          | (iper, fl, ifam) :: neighbourslist -> begin
...
let simplify_path base path =
  ...
  let rec simplify = function
    ...
    | ((i1, (HalfSibling | Sibling | Child)) as x)
      ...
      :: tl ->
      x
      :: (if father i1 = father i2 then (mother i2, Mate) else (father i2, Mate))
      :: simplify tl
...
let rec belongs_to_branch ip dist = function
  | (n, _, ipl) :: lens ->
    if n = dist && List.mem ip ipl then true else belongs_to_branch ip dist lens
  | [] -> false
