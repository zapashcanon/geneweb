...
let fix_pevents ?report base pp =
  ...
  let empty_bi =
    (Adef.cdate_None, Gwdb.empty_string, Gwdb.empty_string, Gwdb.empty_string)
  in
  let empty_bp =
    (Adef.cdate_None, Gwdb.empty_string, Gwdb.empty_string, Gwdb.empty_string)
  in
  ...
  let (burial, burial_place, burial_note, burial_src), pevents =
    ...
    | UnknownBurial -> begin
      ...
      | None ->
        ((UnknownBurial, p.burial_place, p.burial_note, p.burial_src), pevents)
      | Some e ->
    ...
    | Cremated d ->
      let (d, bu_place, bu_note, bu_src), pevents =
        aux Epers_Cremation d p.burial_place p.burial_note p.burial_src empty_bu
          pevents
      in
      ...
      fun i p ->
        ...
        |> Option.iter @@ fun ifam ->
           ...
           if get_ifam fam = dummy_ifam then begin
             ...
             let children = get_children fam in
             if not @@ Array.mem ip children then begin
          ...
          begin
            fun evt ->
              ...
              for j = 0 to Array.length witn - 1 do
                ...
                if not (List.memq ip (get_related p2)) then begin
  ...
  Gwdb.Collection.iteri
    begin
      fun i per ->
        ...
        let p' =
          Futil.map_person_ps ~fd:(fd None (Some iper)) fp (fs None (Some iper))
            p
        in
