...
let image_txt conf base p =
  ...
  | Some _ | None -> (
    ...
    | Some (`Url url, None) ->
      ...
      {|<br><center><table border="0" style="height:|} ^ string_of_int height
      ^ {|px"><tr align="left"><td>|}
...
let longuest_word_length s =
  ...
  let rec loop maxlen len i =
    match displayed_next_char s i with
    | Some (j, k) ->
      if s.[j] = ' ' then loop (max maxlen len) 0 k else loop maxlen (len + 1) k
    | None -> max maxlen len
  in
  loop 0 0 0
...
let displayed_strip s sz =
  ...
  let rec loop strl dibeg di i =
    ...
    | Some j ->
      if dj - dibeg > sz then
        loop (displayed_sub s dibeg (di - dibeg - 1) :: strl) di (dj + 1) (j + 1)
...
let print_html_table conf hts =
  if Util.p_getenv conf.env "notab" <> Some "on" then begin
        ...
        if ((spouse_on && n.chil <> []) || n.pare = []) && not invert then
          List.fold_left
            (fun list id ->
              ...
              | Right _ -> list )
            [] n.chil
        else if n.chil = [] then try [ List.assq ip spl ] with Not_found -> []
        else []
      in
      List.fold_left
        begin
...
let print_dag_page conf page_title hts next_txt =
  ...
  if (next_txt : Adef.escaped_string :> string) <> "" then begin
      ...
      let outs =
        ...
        | TDitem _ | TDtext _ ->
          let s =
            ...
            if k >= 0 && k < Array.length stra.(j) then
              let s = stra.(j).(k) in
              if (s : Adef.safe_string :> string) = "&nbsp;" then Adef.safe " "
              else s
            else try_add_vbar k (Array.length stra.(j)) hts i col
          in
          let len = displayed_length s in
