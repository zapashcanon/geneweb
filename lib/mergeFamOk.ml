...
let merge_events conf l1 l2 =
        ...
        (fun e1 (mem, l1) ->
          if e1.efam_name = e.efam_name then
            ...
            | Efam_Marriage | Efam_NoMarriage | Efam_NoMention ->
              ...
              else if e.efam_name = e1.efam_name && can_merge_event e e1 then
                let date =
                  field e.efam_date e1.efam_date (( = ) Adef.cdate_None)
                in
                let place = field e.efam_place e1.efam_place (( = ) "") in
                let note = merge_strings e.efam_note "<br>\n" e1.efam_note in
  ...
  let rec merge_events_aux l1 l2 =
    ...
    | e2 :: l2 ->
      ...
      if mem then merge_events_aux l1 l2 else merge_events_aux (l1 @ [ e2 ]) l2
  in
...
let effective_mod_merge conf base o_f1 o_f2 sfam scpl sdes =
  ...
  | Some i2 ->
    ...
    in
    let wl =
      UpdateFamOk.all_checks_family conf base ifam fam cpl des (scpl, sdes, None)
    in
