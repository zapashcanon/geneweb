...
and eval_simple_var conf base env p = function
  ...
  | [ "has_witness" ] -> begin
    match get_env "cnt" env with
    | Vint i ->
      let e = try Some (List.nth p.pevents (i - 1)) with Failure _ -> None in
      begin
        match e with
        ...
        | Some e -> begin
          ...
          | Vint i ->
            ...
            let k =
              ...
              else if i >= 0 && i < 2 && Array.length e.epers_witnesses < 2 then
