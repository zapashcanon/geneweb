      ...
      let rec loop i ext =
        ...
        | Some c ->
          ...
          let c =
            ...
            | _ -> (c, Witness)
          in
          let var_w = "e" ^ string_of_int cnt ^ "_ins_witn" ^ string_of_int i in
          begin
        ...
        if !found_death then loop l bi bp de bu
          ...
          let death =
            ...
            | None -> (
              ...
              (* champs sont vides.                               *)
              match death with
              | (DeadYoung | DeadDontKnowWhen | OfCourseDead | DontKnowIfDead)
                as death ->
                death
              | Death _ | NotDead -> DeadDontKnowWhen )
          in
          let de = (death, evt.epers_place, evt.epers_note, evt.epers_src) in
      ...
      List.fold_left
        (fun accu evt ->
          if not !remove_evt then
            if evt.epers_name = Epers_Name "" then begin
              remove_evt := true;
              accu
            end
            else evt :: accu
      ...
      let has_infos, witnesses =
        ...
        | Epers_Birth | Epers_Baptism ->
          ( Adef.od_of_cdate e.epers_date <> None
            || e.epers_place <> "" || e.epers_reason <> "" || e.epers_note <> ""
            || e.epers_src <> ""
...
let update_relations_of_related base ip old_related =
  List.iter
    (fun ip1 ->
      ...
      in
      if rparents_are_different || pevents_are_different then begin
...
let print_mod_ok conf base wl pgl p ofn osn oocc =
  ...
  if List.length !removed_string > 0 then begin
  ...
  if pgl <> [] && (ofn <> nfn || osn <> nsn || oocc <> nocc) then begin
    Output.print_sstring conf
