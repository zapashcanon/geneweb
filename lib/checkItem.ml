              ...
              let rel, changed, not_changed =
                ...
                | true, false ->
                  if get_sex p_ref = Female then
                    ...
                    | None ->
                      let rel = { rel with r_fath = None; r_moth = p_index } in
                      (rel, changed + 1, not_changed)
                ...
                | false, true ->
                  if get_sex p_ref = Male then
                    ...
                    | None ->
                      let rel = { rel with r_moth = None; r_fath = p_index } in
                      (rel, changed + 1, not_changed)
                  else (rel, changed, not_changed)
...
let possible_father warning x father =
  match Adef.od_of_cdate (get_birth x) with
  | Some (Dgreg (d1, _)) when d1.prec <> Before -> begin
...
let check_witness_pevents_aux warning origin evt date b d p witness_kind =
  ...
  | _, Some (Dgreg (d3, _)) when strictly_after_dmy date d3 ->
    if
      witness_kind <> Def.Witness_Mentioned && witness_kind <> Def.Witness_Other
...
let check_parent_marriage_age warning fam p =
  let rec loop = function
    | [] -> ()
    | { efam_name = Efam_Marriage | Efam_PACS; efam_date; _ } :: list -> begin
      match Adef.od_of_cdate efam_date with
      | Some (Dgreg (g2, _) as d2) -> begin
        match Date.date_of_death (get_death p) with
        | Some d1 when strictly_after d2 d1 -> warning (MarriageDateAfterDeath p)
        | _ -> (
          match Adef.od_of_cdate (get_birth p) with
          | Some (Dgreg (g1, _) as d1) ->
            if strictly_before d2 d1 then warning (MarriageDateBeforeBirth p)
            else
              Date.time_elapsed_opt g1 g2
              |> Option.iter @@ fun e ->
          ...
          begin
            fun e ->
              ...
              | Some (Dgreg (date, _)) ->
                ...
                if is_witness then
                  let witness_kind =
                    if only_mentioned then Def.Witness_Mentioned
                    else Def.Witness
                  in
                  check_witness_fevents_aux warning f e date birth_date
                    death_date p witness_kind
              | _ -> ()
          end
          (get_fevents f)
