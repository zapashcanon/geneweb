...
(** [Description] : Renvoie le chemin vers le fichier de template passé en
    ...
    - string : le chemin vers le fichier de template [Rem] : Exporté en clair
      hors de ce module. *)
                  ...
                  let s =
                    let rec loop vl len i =
                      ...
                      then
                        ...
                        | [] ->
                          Buff.get len ^ String.sub s i (String.length s - i)
        ...
        | Out -> (
          ...
          | None -> (
            ...
            | None ->
              ...
              in
              if s.[i] = '&' && not (followed_by_ident_semi s (i + 1)) then
                Buffer.add_string buff "&amp;"
              else Buffer.add_char buff s.[i];
              loop tt (i + 1) ) )
...
let print_alphab_list conf crit print_elem liste =
  ...
  if len > menu_threshold then begin
      ...
      List.fold_left
        (fun last e ->
          ...
          let same_than_last =
            match last with Some t1 -> t = t1 | _ -> false
          in
          if len > menu_threshold || is_number t then begin
            begin
              match last with
              | Some _ ->
                if not same_than_last then
            ...
            if not same_than_last then begin
...
let child_of_parent conf base p =
  ...
  | Some (fath, moth) ->
    ...
    let s = (s :> string) in
    transl_a_of_gr_eq_gen_lev conf (transl_nth conf "son/daughter/child" is) s s
...
let relation_type_text conf t n =
  ...
  | Adoption ->
    ...
    |> Adef.safe
  | Recognition ->
    transl_nth conf "recognizing father/recognizing mother/recognizing parents"
      n
    |> Adef.safe
  | CandidateParent ->
  ...
  | FosterParent ->
    transl_nth conf "foster father/foster mother/foster parents" n |> Adef.safe
      ...
      Array.iter
        (fun ip ->
          if ip = get_iper p then ()
            ...
            Array.iter
              (fun ifam ->
                if Array.length (get_children (foi base ifam)) > 0 then raise Ok
                )
  ...
  let len_list =
    let rec loop rlen_list cnt col accu len list =
      if col > ncol then List.rev rlen_list
        ...
        let cnt = cnt + 1 in
        if accu > len && (not is_last) && !kind = Elem then begin
...
(** [Description] : Affiche la référence d'une personne [Args] :
