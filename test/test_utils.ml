...
let mutil_start_with _ =
  assert_raises (Invalid_argument "start_with") (fun () ->
      Mutil.start_with "foo" (-1) "foo" );
  assert_raises (Invalid_argument "start_with") (fun () ->
      Mutil.start_with "foo" 4 "foo" );
  ...
  assert_bool "Mutil.start_with \"\" 0 \"foo\"" (Mutil.start_with "" 0 "foo")
...
let name_title _ =
  let test exp =
    List.iter (fun s -> assert_equal ~printer:(fun s -> s) exp (Name.title s))
  in
  test "Jean-Baptiste"
...
let util_name_with_roman_number _ =
  let test r a =
    let printer = function
      ...
      | None -> "None"
    in
    assert_equal ~printer r (Util.name_with_roman_number a)
  in
...
let util_safe_html _ =
  assert_equal ~printer:printer_safe
...
let util_escape_html _ =
  assert_equal ~printer:printer_escaped
    (Adef.escaped
       {|&#60;a href=&#34;mailto:jean@dupond.net&#34;&#62;jean@dupond.net&#60;/a&#62; - le 1 &#38;amp; 2|} )
    (Util.escape_html
       {|<a href="mailto:jean@dupond.net">jean@dupond.net</a> - le 1 &amp; 2|} )
...
let suite =
  ...
  ; "Name" >::: [ "name_title" >:: name_title ]
  ; "Utf8" >::: [ "utf8_sub" >:: utf8_sub ]
  ; "Util"
